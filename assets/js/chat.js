...
function chargeMessages(){
        ...
        if (this.readyState == 4){
            if(this.status == 200){
                ...
                for(let message of messages){
                    ...
                    let dateMessage = new Date(message.date)

                    // On ajoute le contenu avant le contenu actuel de discussion
                    discussion.innerHTML = `<p>${message.name} a écrit le ${dateMessage.toLocaleString()} : ${message.message}</p>` + discussion.innerHTML
    ...
    // On ouvre la requête
    xmlhttp.open("GET", "ajax/listMessages.php?lastId="+lastId)
...
/**
 * Cette fonction envoie le message en ajax à un fichier addChat.php
 */
function ajoutMessage(){
    ...
    if(message != ""){
        ...
        // On ouvre la requête
        xmlhttp.open("POST", "ajax/addChat.php")
