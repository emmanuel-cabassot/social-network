    ...
    if (xhr.readyState === 4 || xhr.status == 201) {
      ...
      listPost.forEach((post) => {
        ...
        optionPost.addEventListener("click", function (e) {
          optionClick = document.querySelector(
            "#modify-showPost-option" + post.id_post
          );
          ...
          optionClick.addEventListener("click", function (e) {
            ...
            if (confirm(textConfirm)) {
              if (textConfirm == "Voulez-vous supprimer votre post?") {
                addClassHidden = document.querySelector(
                  "#showOnePost" + post.id_post
                );
        ...
        textareaComment = document.querySelector(
          `#add-comment-textarea${post.id_post}`
        );
        textareaComment.addEventListener("keydown", function (e) {
          if (e.keyCode == 13 && !e.shiftKey) {
            ...
            if (textValue.length > 0) {
              ...
              xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 || xhr.status == 201) {
                  ...
                  newComment.innerHTML = `
                                ...
                                `;
                  elementParent = document.querySelector(
                    "#container-comment" + post.id_post
                  );
              ...
              let showHiddenComment = document.querySelector(
                "#container-comment" + post.id_post
              );
        ...
        /* Bouton commenter */
        boutonCommenter = document.querySelector(
          "#comment-action" + post.id_post
        );
        boutonCommenter.addEventListener("click", function (e) {
          ShowHiddenTextarea = document.querySelector(
            "#add-comment" + post.id_post
          );
          ...
          if (ShowHiddenTextarea.classList.contains("show")) {
            ...
            ShowHiddenTextareaFocus = document.querySelector(
              `#add-comment${post.id_post} textarea`
            );
        ...
        BoutonNbCommentaires = document.querySelector(
          `#commentCount${post.id_post}`
        );
        BoutonNbCommentaires.addEventListener("click", function (e) {
          showHiddenComment = document.querySelector(
            "#container-comment" + post.id_post
          );
          ShowHiddenTextarea = document.querySelector(
            "#add-comment" + post.id_post
        ...
        if (post.countComment > 0) {
          ...
          postComments.forEach((comment) => {
            ...
            avatarUserComment = document.querySelector(
              "#user-photo-comment-showPost" +
                post.id_post +
                "-" +
                comment.id_comment_post
            );
            avatarUserComment.addEventListener("click", function (e) {
              document.location.href =
                "viewFriend.php?id_friend=" + post.id_user;
            });

            /* Apparition des 3 petits points au survol du commentaire */
            survolComment = document.querySelector(
              "#view-comment-showPost" +
                post.id_post +
                "-" +
                comment.id_comment_post
            );
            survolComment.addEventListener("mouseover", function (e) {
              points = document.querySelector(
                "#option-comment" + post.id_post + "-" + comment.id_comment_post
              );
            ...
            points.addEventListener("click", function (e) {
              apparition = document.querySelector(
                "#option-comment-click" +
                  post.id_post +
                  "-" +
                  comment.id_comment_post
              );
              ...
              apparition.addEventListener("click", function (e) {
                ...
                if (confirm(textConfirm)) {
                  if (
                    ...
                    hiddenCommentaire = document.querySelector(
                      "#view-comment-showPost" +
                        post.id_post +
                        "-" +
                        comment.id_comment_post
                  ...
                  xhr.open(
                    "POST",
                    "../api/controllers/deleteSignalerComment.php"
                  );
                  xhr.setRequestHeader("Content-Type", "text/plain");
                  xhr.responseType = "json";
                  xhr.send(JSON.stringify(data));
...
function likeShowPost(like, dislike, post_id) {
  if (like === 0) {
    ...
    if (dislike === 1) {
      ...
      countDislike = document.querySelector(
        "#dislikeCount" + post_id + " span"
      );
...
function check(id_post) {
  let hauteur = document.querySelector(
    "#add-comment-textarea" + id_post
  ).scrollHeight;
  let val = document.querySelector("#add-comment-textarea" + id_post).value;

  if (val === "") {
    document.querySelector("#add-comment-textarea" + id_post).style.height =
      "38px";
  } else {
    document.querySelector("#add-comment-textarea" + id_post).style.height =
      hauteur + "px";
