...
class Group{
    ...
    function suggestGroup($id_user){
        ...
        $query = 'SELECT DISTINCT belong.id_group, name_group, description, img_group, id_user_create FROM groupe  JOIN belong ON belong.id_group = groupe.id_group WHERE id_user IN (SELECT id_followed FROM `friend` WHERE id_follower = :id_user UNION SELECT id_follower FROM friend WHERE id_followed = :id_user) AND groupe.id_group not IN (SELECT id_group FROM belong where id_user= :id_user) ORDER BY RAND() LIMIT 3';
        $stmt = $this->conn->prepare($query);
         $stmt->bindParam(":id_user", $id_user);
        // execute query
        $stmt->execute();
