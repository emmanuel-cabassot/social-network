...
class Friend
{
    ...
    {
        // select all query
        $query = 'Select id_user as id_friend, name, lastname, avatar, city, country from users LEFT JOIN friend ON id_user= id_follower where id_followed IN(SELECT users.id_user from users INNER JOIN (select id_follower as ami from friend where id_followed=:id_user union all SELECT id_followed as ami from friend where id_follower=:id_user) as f on users.id_user= f.ami) and id_follower <>:id_user and id_follower not in (select id_followed from friend where id_follower= :id_user) union Select id_user as id_friend, name, lastname, avatar, city, country from users LEFT JOIN friend ON id_user= id_followed where id_follower in (SELECT users.id_user from users INNER JOIN (select id_follower as ami from friend where id_followed=:id_user union all SELECT id_followed as ami from friend where id_follower=:id_user) as f on users.id_user= f.ami) and id_followed <>:id_user and id_followed not in (select id_follower from friend where id_followed= :id_user) ';
        $stmt = $this->conn->prepare($query);
